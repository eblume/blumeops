...
detached `darwin-rebuild switch`) from the host-mode indri runner. The run
is fire-and-forget: green means the switch launched; the outcome is
`/var/log/indri-rebuild/<sha>.status` on indri ([[provision]]). The full
provision (roles, `op read`) stays a human window.
