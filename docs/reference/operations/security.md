...
| paperless (paperless) | local `blumeops/paperless` (nix, 3.1.3) | non-root image | `User` in `containers/paperless/`; **no fsGroup** — it is pod-scoped and would chown the NFS media PVC recursively at every mount, and the emptyDir data volumes are 0777-writable as-is; media PVC is NFS (sifaka:/volume1/paperless) → one-time pre-chown to uid/gid 1000 |
| teslamate (teslamate) | local `blumeops/teslamate` (nix, mix release 4.2.0) | non-root image | `User` in `containers/teslamate/` + a writable uid-owned `/opt/app` created in the image so HOME and SRTM_CACHE point at a directory that exists; stateless otherwise |
