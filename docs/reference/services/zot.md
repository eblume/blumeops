...
| `ci-artifacts` group (`ci-zot`) | read, create | The base CI push identity — CI push for `build-container` (new tags only, no overwrite/delete); eblume/blumeops#1039 step 2b |
...
CI authenticates with a zot API key generated from the `ci-zot` service account's OIDC session. The key is stored in the `Forgejo Secrets` 1Password item (field `ci-zot-api`) and mirrored to `blumeops-ci/ci-zot` (`api-key`); workflows `op read` it at job time with `BLUMEOPS_CI_OP_TOKEN` — it is not a Forgejo Actions secret.
...
```fish
mise run zot-apikey-rotate ci-zot        # or ci-zot-talos, ci-zot-horkos, horkos-zot
mise run zot-apikey-rotate ci-zot --dry-run   # just prove the current key still works
```
...
**ci-zot cutover (done, 2026-09-17).** `ci-zot` is now the base CI push
identity: the flip PR (eblume/blumeops#1156, commit c2aba52f) moved
`build-container.yaml` to it, the ceremony renamed the consumer item and
rotated (build-container run 4359 pushed prowler as `ci-zot`), and this
cleanup retires the previous identity.

The task reads the current key from the `Forgejo Secrets` item (blumeops
vault; fields `ci-zot-api`, `ci-zot-talos-api`, `ci-zot-horkos-api`,
`horkos-zot-api`), mints a new one, proves the new key authenticates, writes the
master copy back, writes the consumer copy — `blumeops-ci/ci-zot`
(`api-key`) for `ci-zot`, the `ZOT_PUSH_API_KEY` Actions secret on
...
1. In the Authentik admin UI, impersonate the identity (`ci-zot`, `ci-zot-talos`, `ci-zot-horkos` or `horkos-zot`)
