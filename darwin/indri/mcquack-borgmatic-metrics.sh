...
EOF
    ...
    # Collect per-source-directory sizes. This pulls the full file manifest of
    # the latest archive, which is cheap on a local repo but a heavy transfer on
    # a remote (ssh://) one — 100k+ path entries over the internet, every run.
    # We poll remote repos hourly for the light info/list metrics above (up,
    # timestamp, repo sizes), but skip this per-source breakdown for them.
